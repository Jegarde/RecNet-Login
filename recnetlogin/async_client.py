...
class RecNetLoginAsync(BaseClient):
    ...
    async def __login(self) -> None:
        async with self.__session.get(ApiInfo.URL) as resp:
        ...
        async with self.__session.post(ApiInfo.URL + ApiInfo.PARAMS, data=body, allow_redirects=True, headers=ApiInfo.HEADERS) as resp:
            try:
    ...
    async def __login_2fa(self) -> None:
        ...
        async with self.__session.get(ApiInfo.TWOFA_URL) as resp:
            html = await resp.text()
            rvt = parse_rvt(html=html)
